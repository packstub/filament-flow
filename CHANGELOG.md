...
- **New from template** shows the templates as cards — the area, the name, what the workflow does and its steps in canvas order, coloured like the nodes — with the areas as a filter above them while more than one is offered, in a wider modal; the picked card is outlined. The area no longer prefixes the name. `TemplatePicker` is the field behind it, a `Radio` whose options are the templates.
- Tables: the Workflows list keeps the description to one line (the rest in a tooltip) and lets **Triggers** be hidden; the error column of the Runs tab and page is shorter, with the full message in a tooltip; **Secrets** shows the key with its `{{ secrets.key }}` placeholder underneath, copied on click.
- **Approvals** reads in three glances instead of seven columns: the request takes the room, with its details under the title; the workflow carries the record under its name, the status who decided, the request date the deadline. The record, **Expires** and **Decided by** remain as columns, hidden until toggled on. **Reject** is outlined and **Cancel** an icon button with a tooltip. The payload and output blocks of a run's details use the panel's grey code style instead of black.
...
- The plugin's stylesheet no longer touches the rest of the panel: its utilities are scoped to the canvas and the plugin's own views (`.fi-flow-builder`, `.fi-flow`), so a `text-gray-700 dark:text-gray-300` elsewhere in the panel keeps its dark-mode colour ([#32](https://github.com/packstub/filament-flow/pull/32), thanks @grafst).
...
- **Template picker**: the «Category — Name» prefix is only shown while more than one category is offered; an install that switched the built-in templates off (`withoutBuiltInTemplates()`) and ships a single category now sees plain names.
...
- **Sidebar groups**: the sidebar now groups nodes by `Node::getCategory()` — `triggers` / `actions` / `conditions` after the base class — and Ask AI opens a fourth group, **AI** (sparkles icon, teal on the canvas), shown only while an AI node is offered; the node's type, handles and runtime behaviour are unchanged and nothing new is stored in a workflow. Your own nodes can declare a group (`return 'crm';`) with its name and description under the `builder` translations (`builder.crm`, `builder.crm_description`); a group without one shows its key capitalised. The registry's `toArray()` carries `category` per node.

### Changed

- Tooling: the Playwright smoke test covers the real Ask AI node when `packstub/agents` is installed next to the workbench (CI's canvas job installs it): the AI group in the sidebar, the teal node, its settings from the engine's model picker, a saved workflow and a dry run that names the model it would ask. Skipped when the engine is absent.
