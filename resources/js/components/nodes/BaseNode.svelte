...
<div class="relative group" role="presentation">
    <!-- Node Container -->
    <div
        class="min-w-[180px] max-w-[240px] bg-white dark:bg-slate-900 rounded-xl shadow-sm border {theme.border} overflow-hidden transition-all duration-200 {selected
    ...
    >
        ...
        <div class="p-3 {theme.bg}">
            {#if data.description}
                <p
                    class="text-[10px] leading-relaxed text-slate-500 dark:text-slate-400 mb-2 italic"
                >
                    {data.description}
                </p>
            {/if}
